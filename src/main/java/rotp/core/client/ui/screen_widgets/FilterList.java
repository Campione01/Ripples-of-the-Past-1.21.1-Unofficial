package rotp.core.client.ui.screen_widgets;

import java.util.ArrayList;
import java.util.EnumMap;
import java.util.List;
import java.util.Map;
import java.util.function.Consumer;
import java.util.function.Predicate;

import javax.annotation.Nullable;

import rotp.core.client.ui.utils.BlitFloat;
import rotp.core.core.JojoMod;
import rotp.core.util.enums.Direction2D;
import com.mojang.blaze3d.vertex.PoseStack;

import net.minecraft.client.Minecraft;
import net.minecraft.client.gui.GuiGraphics;
import net.minecraft.client.gui.components.events.GuiEventListener;
import net.minecraft.client.gui.narration.NarratableEntry;
import net.minecraft.client.gui.narration.NarrationElementOutput;
import net.minecraft.client.resources.sounds.SimpleSoundInstance;
import net.minecraft.resources.ResourceLocation;
import net.minecraft.sounds.SoundEvents;
import net.minecraft.util.Mth;

public class FilterList<T extends FilterList.Entry> implements GuiEventListener, NarratableEntry {
	protected int topEntry = 0;
	protected final List<T> allEntries;
	protected List<T> renderedEntries;
	public boolean visible = true;

	@Nullable protected Predicate<T> filter;

	protected int x;
	protected int y;
	protected int yTop;
	protected int yBottom;
	protected int width;
	protected int height;
	protected int maxHeight;
	protected int maxEntriesRendered;
	protected int entriesRenderedCount;
	protected final int entryHeight;
	protected final AlignmentY alignmentY;

	public FilterList(List<T> entries, 
			int x, int yTop, int width, int yBottom, int entryHeight, AlignmentY alignmentY) {
		this.x = x;
		this.yTop = yTop;
		this.yBottom = yBottom;
		this.width = width;
		this.entryHeight = entryHeight;
		this.alignmentY = alignmentY;

		this.allEntries = entries;
		this.renderedEntries = new ArrayList<>(allEntries);
		updateMaxHeight();
	}

	public void setYBottom(int yBottom) {
		this.yBottom = yBottom;
		updateMaxHeight();
	}

	public int getYBottom() {
		return yBottom;
	}

	protected void updateMaxHeight() {
		this.maxHeight = yBottom - yTop;
		this.maxEntriesRendered = Math.max(maxHeight / entryHeight, 1);
		this.entriesRenderedCount = Math.min(renderedEntries.size(), maxEntriesRendered);
		this.height = entriesRenderedCount * entryHeight + 32;
		switch (alignmentY) {
			case TOP:
				this.y = yTop - 16;
				break;
			case BOTTOM:
				this.y = yBottom - height;
				break;
			default:
				throw new IllegalStateException();
		}
		updatePositions();
	}

	protected void updatePositions() {
		int i = -topEntry;
		for (T entry : renderedEntries) {
			entry.setY(this.y + 16 + (int) i * entryHeight);
			i++;
		}
		setTopEntryIndex(topEntry);
	}

	public void render(GuiGraphics guiGraphics, Minecraft mc, 
			int mouseX, int mouseY, float partialTick) {
		if (!visible) return;

		PoseStack poseStack = guiGraphics.pose();
		BlitFloat.blit(poseStack, mc, 
				getScrollUpState(mouseX, mouseY).sprites.get(Direction2D.UP), 
				x + width / 2 - 8, y, 16, 16, 0, 
				BlitFloat.NO_TINT);

		for (int i = 0; i < entriesRenderedCount && i + topEntry < renderedEntries.size(); i++) {
			T entry = renderedEntries.get(i + topEntry);
			entry.render(guiGraphics, mc, mouseX, mouseY, partialTick);
		}

		BlitFloat.blit(poseStack, mc, 
				getScrollDownState(mouseX, mouseY).sprites.get(Direction2D.DOWN), 
				x + width / 2 - 8, y + height - 16, 16, 16, 0, 
				BlitFloat.NO_TINT);
	}

	protected int getMaxTopEntryIndex() {
		return Math.max(renderedEntries.size() - maxEntriesRendered, 0);
	}


	public void setFilter(@Nullable Predicate<T> filter) {
		this.filter = filter; // TODO fix filter
		this.renderedEntries = new ArrayList<>();
		for (T entry : allEntries) {
			boolean visible = filter == null || filter.test(entry);
			if (visible) {
				renderedEntries.add(entry);
			}
			entry.setVisible(visible);
		}

		updateMaxHeight();
	}


	protected ScrollButtonState getScrollButtonState(int mouseX, int mouseY, int x, int y, boolean isEnabled) {
		if (!isEnabled) {
			return ScrollButtonState.DISABLED;
		}
		boolean isHovered = mouseX > x + 3 && mouseX < x + 14 && mouseY > y + 4 && mouseY < y + 12;
		return isHovered ? ScrollButtonState.HOVERED : ScrollButtonState.NORMAL;
	}

	protected ScrollButtonState getScrollUpState(int mouseX, int mouseY) { // FIXME make these separate buttons
		return getScrollButtonState(mouseX, mouseY, x + width / 2 - 8, y, topEntry > 0);
	}

	protected ScrollButtonState getScrollDownState(int mouseX, int mouseY) {
		return getScrollButtonState(mouseX, mouseY, x + width / 2 - 8, y + height - 16, topEntry < getMaxTopEntryIndex());
	}

	public static enum ScrollButtonState {
		NORMAL(""),
		DISABLED("_inactive"),
		HOVERED("_hovered");
		
		public final Map<Direction2D, ResourceLocation> sprites;

		private ScrollButtonState(String postfix) {
			this.sprites = new EnumMap<>(Direction2D.class);
			for (Direction2D dir : Direction2D.values()) {
				sprites.put(dir, JojoMod.resLoc("textures/gui/sprites/arrow/" + dir.name().toLowerCase() + postfix + ".png"));
			}
		}
	}

	@Override
	public boolean mouseClicked(double mouseX, double mouseY, int buttonId) {
		if (!visible) return false;
		if (getScrollUpState((int) mouseX, (int) mouseY) == ScrollButtonState.HOVERED) {
			setTopEntryIndex(topEntry - 1);
			Minecraft.getInstance().getSoundManager().play(SimpleSoundInstance.forUI(SoundEvents.UI_BUTTON_CLICK, 1.0F));
			return true;
		}
		if (getScrollDownState((int) mouseX, (int) mouseY) == ScrollButtonState.HOVERED) {
			setTopEntryIndex(topEntry + 1);
			Minecraft.getInstance().getSoundManager().play(SimpleSoundInstance.forUI(SoundEvents.UI_BUTTON_CLICK, 1.0F));
			return true;
		}

		for (int i = 0; i < entriesRenderedCount; i++) {
			T entry = renderedEntries.get(i + topEntry);
			if (entry.mouseClicked(mouseX, mouseY, buttonId)) {
				return true;
			}
		}

		return false;
	}

	@Override
	public boolean mouseScrolled(double mouseX, double mouseY, double scrollX, double scrollY) {
		if (!visible) return false;

		if (scrollY < 0) {
			setTopEntryIndex(topEntry + 1);
		}
		else if (scrollY > 0) {
			setTopEntryIndex(topEntry - 1);
		}
		return true;
	}

	@Override
	public boolean isMouseOver(double mouseX, double mouseY) {
		return mouseX > x && mouseX < x + width && mouseY > y && mouseY < y + height;
	}

	protected void setTopEntryIndex(int index) {
		index = Mth.clamp(index, 0, getMaxTopEntryIndex());
		if (this.topEntry != index) {
			int diff = index - this.topEntry;
			this.topEntry = index;
			renderedEntries.forEach(entry -> entry.addY(-diff * entryHeight));
		}
	}

	public void forEachRendered(Consumer<T> render) {
		for (int i = 0; i < entriesRenderedCount && i + topEntry < renderedEntries.size(); i++) {
			T entry = renderedEntries.get(i + topEntry);
			render.accept(entry);
		}
	}


	public static enum AlignmentY {
		TOP,
		BOTTOM;
	}

	public static interface Entry {
		void setY(int y);
		void addY(int addY);
		void setVisible(boolean isVisible);
		void render(GuiGraphics guiGraphics, Minecraft mc, int mouseX, int mouseY, float partialTick);
		boolean mouseClicked(double mouseX, double mouseY, int buttonId);
	}


	@Override
	public void updateNarration(NarrationElementOutput narrationElementOutput) {
	}

	@Override
	public NarrationPriority narrationPriority() {
		return NarrationPriority.NONE;
	}

	@Override
	public void setFocused(boolean focused) {
	}

	@Override
	public boolean isFocused() {
		return false;
	}
}

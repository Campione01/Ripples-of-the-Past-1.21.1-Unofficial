...
/** Bounds repeated client network stack traces without changing packet handling. */
public final class ClientNetworkFailureLogLimiter {
	...
	private static final ClientNetworkFailureLogLimiter INSTANCE =
			new ClientNetworkFailureLogLimiter(
	...
	public static Decision acquire(
			String path,
			String subject,
			Class<? extends Throwable> errorType) {
		return INSTANCE.acquire(path, subject, errorType, System.nanoTime());
	}

	ClientNetworkFailureLogLimiter(
		...
		if (maxClassifications <= 0 || classificationWindowNanos <= 0L
			...
			throw new IllegalArgumentException("Invalid client network failure log limits");
		}

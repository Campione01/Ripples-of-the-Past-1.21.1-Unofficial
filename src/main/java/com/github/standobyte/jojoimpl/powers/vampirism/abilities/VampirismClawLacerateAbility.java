...
public class VampirismClawLacerateAbility extends VampirismActionAbility {

	public VampirismClawLacerateAbility(AbilityType<?> abilityType, AbilityId abilityId) {
		super(abilityType, abilityId, Integer.MAX_VALUE, 45.0F, ClawLacerateInstance::new);
		setDefaultPhaseLength(ActionPhase.PERFORM, 12);
	...
	public static class ClawLacerateInstance extends EntityActionInstance {
		...
		public void actionTick() {
			...
			if (tick == 3 && level().isClientSide()) {
				level().playLocalSound(user.getX(), user.getEyeY(), user.getZ(),
						ModSoundEvents.VAMPIRE_SWIPE.get(), user.getSoundSource(), 1.0F, 1.25F, false);
		...
		private static void hitEntity(Level level, LivingEntity user, LivingEntity target) {
			...
			if (target.hurt(damageSource, damage)) {
				level.playSound(null, target, ModSoundEvents.VAMPIRE_CLAW_LACERATE.get(),
